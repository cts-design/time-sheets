...
class KioskButtonsController extends AppController {
	...
	$count = $this->KioskButton->find('count', array(
	    'conditions' => array(
		'KioskButton.id' => $data['KioskButton']['id'],
	...
	if($count > 0 ) {
	    ...
	    $this->redirect(array('action' => 'index', $data['KioskButton']['kiosk_id']));
	}
	...
	if($masterButton['MasterKioskButton']['parent_id'] != '') {
	    ...
	    $possibleParentCount = $this->KioskButton->find('count', array('conditions' => array(
		'KioskButton.id' => $data['KioskButton']['parent_id'],
		'KioskButton.kiosk_id' => $data['KioskButton']['kiosk_id'],
		'KioskButton.status' => 0
	    )));
	...
	$button = $this->KioskButton->find('first', array('conditions' => array(
	    'KioskButton.id' => $data['KioskButton']['id'],
	    'KioskButton.kiosk_id' => $data['KioskButton']['kiosk_id']
	)));
	...
	if($button['KioskButton']['status'] == 1) {
		...
		$this->Transaction->createUserTransaction('Kiosk', null, null,
			'Enabled kiosk button ' . $button['MasterKioskButton']['name']);
	...
	$data['KioskButton']['button_id'] = $button['KioskButton']['button_id'];
	$possibleChildrenCount = $this->KioskButton->find('count', array('conditions' => array(
	...
	if($this->KioskButton->save($data)) {
		$this->Transaction->createUserTransaction('Kiosk', null, null,
			'Disabled kiosk button ' . $button['MasterKioskButton']['name']);
	...
	foreach($this->params['url'] as $k => $v) {
	    $data['KioskButton'][$v] = array('button_id' => $v, 'order' => $k);
	}
	...
	$masterButtons = $this->MasterKioskButton->find('threaded', array(
		    'conditions' => array(
			'MasterKioskButton.deleted !=' => 1),
		    'order' => array('MasterKioskButton.id', 'MasterKioskButton.id DESC')));
	$masterButtonNames = $this->MasterKioskButton->find('list', array(
		    'conditions' => array(
			'MasterKioskButton.deleted !=' => 1)
	    ));
	$masterButtonParentIds = $this->MasterKioskButton->find('list', array(
		    'conditions' => array(
			'MasterKioskButton.deleted !=' => 1),
	...
	$vars  = array(
	    'masterButtonParentIds' => $masterButtonParentIds,
