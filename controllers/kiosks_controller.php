...
class KiosksController extends AppController {
	...
	parent::beforeFilter();
	$this->Auth->allow(array('selfSign'));
	$this->Cookie->name = 'self_sign';
    }

    function admin_index() {
	$this->Kiosk->recursive = 0;
	$this->paginate = array('limit' => Configure::read('Pagination.kiosk.limit'),
	...
	if(!empty($this->data)) {
		...
		$this->Transaction->createUserTransaction('Kiosk',
			null, null, 'Edited kiosk ' . $this->data['Kiosk']['location_recognition_name']);
		$this->Session->setFlash(__('The kiosk has been saved', true), 'flash_success');
		$this->redirect(array('action' => 'index'));
	    }
	    else {
	...
	if($this->Kiosk->delete($id)) {
	    ...
	    $this->Transaction->createUserTransaction('Kiosk',
		    null, null, 'Deleted kiosk ' . $kiosk['Kiosk']['location_recognition_name']);
	...
	if(!empty($buttonId)) {
	    ...
	    $possibleChildren = $this->Kiosk->KioskButton->find('all',
			    array('conditions' => array(
		...
		$button = $this->Kiosk->KioskButton->find('first',
				array('conditions' => array(
		...
		if($button['KioskButton']['parent_id'] != null) {
		    $button2 = $this->Kiosk->KioskButton->find('first',
				    array('conditions' => array(
			...
			$count = $this->Kiosk->KioskButton->find('count',
					array('conditions' => array(
						'KioskButton.parent_id' => $buttonId,
						'KioskButton.status' => 0),
					    'order' => array('KioskButton.order' => 'asc')));
	...
	if(!empty($this->data)) {
	    ...
	    if($id) {
		$this->loadModel('User');
		$this->Transaction->createUserTransaction('Self Scan', null,
			$this->data['QueuedDocument']['scanned_location_id'],
			'Self scanned document ID ' . $id);
