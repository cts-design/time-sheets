	...
	initWindow: function(rec) {
		...
		var windowTpl = new Ext.XTemplate(
			'<div class="x-careerseeker-row">',
				'<tpl for="answers">',
					...
					'<p class="left">{date_you_worked_with_the_business_services_team_or_the_website}</p>',
					'<br class="clear" />',
					...
					'<p class="left">',
						'<ul>',
							'<tpl if="food_stamps == 1">Food Stamps (FSET)</tpl>',
							'<tpl if="unemployment_comp == 1">Unemployment Compensation</tpl>',
							'<tpl if="wia == 1">WIA</tpl>',
							'<tpl if="vocational_rehab == 1">Vocational Rehabilitation</tpl>',
							'<tpl if="welfare_transition == 1">Welfare Transition (AFDC/TANF)</tpl>',
							'<tpl if="veterans_services == 1">Veterans Services</tpl>',
							'<tpl if="universal_services == 1">Universal Services (Resource Room, Job Search)</tpl>',
						'</ul>',
					'</p>',
					'<br class="clear" />',
					'<p class="label left">How did you learn about us (please choose one):</p>',
					'<p class="left">{how_did_you_learn_about_(please_choose_one)}</p>',
					'<br class="clear" />',
		...
		if (!this.surveyWindow) {
			this.surveyWindow = new Ext.Window({
				title: 'Career Seeker Survey &ndash; Full Name',
				height: 500,
				width: 650,
				closeAction: 'hide',
				resizable: false,
